...
import { ChevronDown, Info, Plus, Trash2, Zap, Search } from "lucide-react";
...
export default function CreateBotPage() {
    ...
    // --- 🚀 SUBMIT FUNCTION (ตามตัวอย่าง Payload) ---
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Mapping ข้อมูลให้ตรงตาม Payload ตัวอย่าง
            const payload = {
                user_id: userId,
                ...
                pin: auth.pin,
                public: "true", // ค่าเริ่มต้นตามตัวอย่าง
                bot_type: "POLICY",
                strategy_config: {
                    risk: {
        ...
        <div className="min-h-screen bg-white font-kanit">
            ...
            <main className="max-w-5xl mx-auto pt-32 pb-40 px-6">
                <div className="flex flex-col items-center mb-12">
                    <h1 className="text-3xl font-black text-[#6A0DAD] mb-8">Create Your Trading Bot</h1>
                    <div className="flex bg-slate-100 p-1 rounded-xl mb-12 shadow-inner">
                        <button type="button" onClick={() => setActiveTab("Market")} className={`px-10 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === "Market" ? "bg-[#6A0DAD] text-white shadow-lg" : "text-slate-500"}`}>Market</button>
                        <button type="button" onClick={() => setActiveTab("Backtest")} className={`px-10 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === "Backtest" ? "bg-[#6A0DAD] text-white shadow-lg" : "text-slate-500"}`}>Backtest</button>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-16">
                    ...
                    <div className="pt-10 pb-20">
                        <button type="submit" disabled={isSubmitting} className="w-full py-6 bg-[#6A0DAD] text-white text-xl font-black rounded-full shadow-2xl shadow-purple-100 hover:bg-[#5D0CA1] transition-all disabled:bg-slate-300">
