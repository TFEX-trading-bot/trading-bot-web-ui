            ...
            <div className="flex p-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm gap-2">
              <button 
                ...
                className={`px-12 py-3 rounded-[2rem] text-sm font-black uppercase tracking-widest transition-all duration-300 ${
                  activeTab === 'dashboard' ? 'bg-[#8B5CF6] text-white shadow-xl shadow-purple-200' : 'text-slate-400 bg-transparent hover:text-slate-600'
                }`}
              >
                Dashboard
              </button>
              {data?.botType !== "AI" && (
                <button 
                  ...
                  className={`px-12 py-3 rounded-[2rem] text-sm font-black uppercase tracking-widest transition-all duration-300 ${
                    activeTab === 'configuration' ? 'bg-[#8B5CF6] text-white shadow-xl shadow-purple-200' : 'text-slate-400 bg-transparent hover:text-slate-600'
                  }`}
                >
                  Configuration
                </button>
          ...
          {activeTab === 'dashboard' ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                  ...
                  <div className={`p-8 rounded-3xl flex items-center gap-8 border transition-all ${(data?.totalPnL ?? 0) < 0 ? 'bg-rose-50 border-rose-100 text-rose-500' : 'bg-emerald-50 border-emerald-100 text-emerald-500'}`}>
                    ...
                    <div>
                      <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mb-1">Total PnL</p>
                      <p className="text-4xl font-black">{data?.totalPnL.toLocaleString()} ฿</p>
                    </div>
                  </div>
                </div>
            ...
            <section className="animate-in slide-in-from-bottom-8 duration-600 space-y-12 max-w-5xl mx-auto pb-20">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="relative bg-white rounded-2xl shadow-sm border border-slate-100 h-11 min-w-[100px] flex items-center">
                             <select 
                              ...
                              className="appearance-none w-full bg-transparent font-black text-sm outline-none px-4 pr-10 text-slate-900"
                        ...
                        <div key={cond.id} className="grid grid-cols-4 gap-6 items-center">
                          ...
                          <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {cond.rightType === 'INDICATOR' && (
                               <div className="w-12 h-10 bg-slate-100 rounded-xl flex items-center justify-center"><input value={cond.rightPeriod} onChange={e => updateCondition(rule.id, cond.id, 'rightPeriod', e.target.value)} className="bg-transparent w-full text-center text-xs font-bold outline-none" placeholder="Pd" /></div>
                            )}
                          </div>
              ...
              <div className="flex justify-center gap-8 pt-10">
                <button onClick={handleSaveConfig} disabled={isSaving} className="px-20 py-6 bg-[#8B5CF6] text-white text-lg font-black rounded-full shadow-2xl shadow-purple-200 hover:bg-purple-700 transition-all hover:-translate-y-1 active:scale-95 flex items-center gap-4">
